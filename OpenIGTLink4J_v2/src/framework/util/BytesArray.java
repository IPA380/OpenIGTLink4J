...
public class BytesArray {
	...
	/** the byte array to put everything in */
	 ...
	 *** @return The long value of timeStamp
	 **/
	public long putTimeStamp(long timestamp) {
		byte n[] = encodeTimeStamp(timestamp);
		putBytes(n);
		return timestamp;
	}
	...
	public long putTimeStamp() {
		return putTimeStamp(getTimeStamp());
	}
	 ...
	 **/
	public static long getCrc(byte[] buf, int len, long crc) {
		long comp_crc = crc64(buf, len, crc);
		return comp_crc;
	}
	...
	public int putString(String s, int length) {
		int bytesPut = putString(s);
		int remaining = length - bytesPut;
		if (remaining > 0) {
			String emptyString = new String(new char[remaining]);
			putString(emptyString);
		}
		return bytesPut + remaining;
	}
